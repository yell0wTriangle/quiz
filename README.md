...
A static, responsive quiz built from the `IT Questions for Quiz` worksheet in `PhonePe_Debit_Analysis.xlsx`. It has no database or server-side code.
...
To update the question bank later, replace or edit `PhonePe_Debit_Analysis.xlsx`, run `python build_questions.py`, and redeploy the generated `questions.js`.
